...
const ils=x=>x==null?'—':'₪'+Number(x).toLocaleString(undefined,{maximumFractionDigits:2});
...
<h2>הארנק המדומה</h2><div class="grid"><div class="card"><div class="label">שווי הארנק</div><div class="v ${d.wallet.equity>=d.wallet.initial?'up':'down'}">${ils(d.wallet.equity)}</div></div><div class="card"><div class="label">יתרה ממומשת</div><div class="v">${ils(d.wallet.balance)}</div></div><div class="card"><div class="label">כסף פנוי</div><div class="v">${ils(d.wallet.available)}</div></div><div class="card"><div class="label">בעסקאות פתוחות</div><div class="v amber">${ils(d.wallet.locked)}</div></div><div class="card"><div class="label">רווח פתוח</div><div class="v ${d.wallet.unrealized>=0?'up':'down'}">${ils(d.wallet.unrealized)}</div></div><div class="card"><div class="label">הפקדה התחלתית</div><div class="v">${ils(d.wallet.initial)}</div></div></div>
<div class="grid"><div class="card"><div class="label">חלונות שנאספו</div><div class="v">${s.markets||0}</div></div><div class="card"><div class="label">דגימות שוק</div><div class="v">${s.n||0}</div><div class="mini">תצפיות, לא עסקאות</div></div><div class="card"><div class="label">עסקאות מדומות</div><div class="v">${t.n||0}</div></div><div class="card"><div class="label">נסגרו / הצליחו</div><div class="v">${t.settled||0} / ${t.wins||0}</div><div class="mini">Win rate: ${t.win_rate||0}%</div></div><div class="card"><div class="label">P&L סופי</div><div class="v ${(t.pnl||0)>=0?'up':'down'}">${ils(t.pnl||0)}</div><div class="mini">עסקאות שהוכרעו</div></div><div class="card"><div class="label">P&L נוכחי</div><div class="v ${(t.mark_pnl||0)>=0?'up':'down'}">${ils(t.mark_pnl||0)}</div><div class="mini">כולל עסקאות פתוחות</div></div></div>
<section id="analysis"><h2>ניתוח מצטבר</h2><div class="grid"><div class="card"><div class="label">רווחים גולמיים</div><div class="v up">${ils(d.analysis.gross_profit)}</div></div><div class="card"><div class="label">הפסדים גולמיים</div><div class="v down">${ils(-d.analysis.gross_loss)}</div></div><div class="card"><div class="label">Profit Factor</div><div class="v">${d.analysis.profit_factor||'—'}</div></div><div class="card"><div class="label">ממוצע לעסקה</div><div class="v">${ils(d.analysis.avg_pnl)}</div></div><div class="card"><div class="label">חציון לעסקה</div><div class="v">${ils(d.analysis.median_pnl)}</div></div><div class="card"><div class="label">5 הזכיות הגדולות</div><div class="v">${d.analysis.top5_profit_share}%</div><div class="mini">מסך הרווחים הגולמיים</div></div></div><h2>בדיקת יציאה חכמה — מהיום</h2><div class="grid"><div class="card"><div class="label">עסקאות שיצאו</div><div class="v">${d.analysis.managed.trades}</div></div><div class="card"><div class="label">P&L ביציאה חכמה</div><div class="v ${d.analysis.managed.pnl>=0?'up':'down'}">${ils(d.analysis.managed.pnl)}</div></div><div class="card"><div class="label">הצלחה ביציאות</div><div class="v">${d.analysis.managed.win_rate}%</div></div><div class="card"><div class="label">ממוצע ליציאה</div><div class="v">${ils(d.analysis.managed.avg_pnl)}</div></div></div><div class="mini">Take profit 35% · Stop loss 30% · היפוך מודל · יציאה 15 שניות לפני הסיום. ההשוואה מתחילה מהגרסה הנוכחית בלבד.</div><div class="scroll"><table><tr><th>קבוצה</th><th>עסקאות</th><th>הצלחות</th><th>Win rate</th><th>P&L</th></tr>${Object.entries({...d.analysis.by_side,...d.analysis.by_price,...d.analysis.by_edge}).map(([k,x])=>`<tr><td>${k}</td><td>${x.trades}</td><td>${x.wins}</td><td>${x.win_rate}%</td><td class="${x.pnl>=0?'up':'down'}">${ils(x.pnl)}</td></tr>`).join('')}</table></div></section>
<h2>עסקאות מדומות אחרונות</h2><div class="scroll"><table><tr><th>זמן</th><th>צד</th><th>כניסה</th><th>עכשיו</th><th>מודל</th><th>Edge</th><th>מצב</th><th>רווח/הפסד</th></tr>${d.recent.map(x=>`<tr><td>${new Date(x.ts*1000).toLocaleTimeString('he-IL',{hour:'2-digit',minute:'2-digit'})}</td><td class="${x.side==='Up'?'up':'down'}">${x.side}</td><td>${cent(x.fill_price)}</td><td>${cent(x.mark_price)}</td><td>${pct(x.model_p)}</td><td>${pct(x.edge)}</td><td class="status">${x.display_status}</td><td class="${(x.display_pnl||0)>=0?'up':'down'}">${x.display_pnl==null?'—':ils(x.display_pnl)}</td></tr>`).join('')}</table></div>${r.last_error?'<p class="err">'+r.last_error+'</p>':''}`;chart(document.getElementById('btcChart'),rows,['btc','open_btc'],['#4ea1ff','#ffd166']);chart(document.getElementById('polyChart'),rows,['up_ask','down_ask','p_up'],['#45e69a','#ff6575','#b584ff'],true)}
