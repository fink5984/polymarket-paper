...
def stats():
    ...
    td=dict(t); td["win_rate"] = round(100*(td["wins"] or 0)/(td["settled"] or 1),1)
...
function chart(canvas, rows, keys, colors, probability=false){let ctx=canvas.getContext('2d'),w=canvas.clientWidth,h=canvas.clientHeight,d=devicePixelRatio||1;canvas.width=w*d;canvas.height=h*d;ctx.scale(d,d);ctx.clearRect(0,0,w,h);let pad=34;if(rows.length<2){ctx.fillStyle='#8499b4';ctx.fillText('ממתין לעוד נתונים…',w/2-50,h/2);return}let vals=rows.flatMap(r=>keys.map(k=>r[k]).filter(x=>x!=null)),mn=probability?0:Math.min(...vals),mx=probability?1:Math.max(...vals);if(mx==mn){mx+=1;mn-=1}ctx.strokeStyle='#213650';ctx.fillStyle='#8499b4';ctx.font='11px system-ui';for(let i=0;i<=4;i++){let y=pad+(h-2*pad)*i/4;ctx.beginPath();ctx.moveTo(pad,y);ctx.lineTo(w-8,y);ctx.stroke();let value=mx-(mx-mn)*i/4;ctx.fillText(probability?(value*100).toFixed(0)+'¢':value.toFixed(0),2,y+4)}keys.forEach((k,ki)=>{ctx.strokeStyle=colors[ki];ctx.lineWidth=2;ctx.beginPath();rows.forEach((r,i)=>{if(r[k]==null)return;let x=pad+(w-pad-10)*i/(rows.length-1),y=pad+(h-2*pad)*(mx-r[k])/(mx-mn);i?ctx.lineTo(x,y):ctx.moveTo(x,y)});ctx.stroke()})}
...
<div class="charts"><div class="panel"><h2>מחיר BTC בתוך החלון</h2><div class="legend"><span><i class="sw" style="background:#4ea1ff"></i>BTC</span><span><i class="sw" style="background:#ffd166"></i>מחיר פתיחה</span></div><div class="chartbox"><canvas id="btcChart"></canvas></div></div><div class="panel"><h2>Polymarket מול המודל</h2><div class="legend"><span><i class="sw" style="background:#45e69a"></i>UP Ask</span><span><i class="sw" style="background:#ff6575"></i>DOWN Ask</span><span><i class="sw" style="background:#b584ff"></i>Model UP</span></div><div class="bar"><span style="width:${(up||.5)*100}%"></span></div><div class="chartbox"><canvas id="polyChart"></canvas></div></div></div>
<div class="grid"><div class="card"><div class="label">חלונות שנאספו</div><div class="v">${s.markets||0}</div></div><div class="card"><div class="label">דגימות לייב</div><div class="v">${s.n||0}</div></div><div class="card"><div class="label">עסקאות מדומות</div><div class="v">${t.n||0}</div></div><div class="card"><div class="label">נסגרו / הצליחו</div><div class="v">${t.settled||0} / ${t.wins||0}</div><div class="mini">Win rate: ${t.win_rate||0}%</div></div><div class="card"><div class="label">P&L מדומה</div><div class="v ${(t.pnl||0)>=0?'up':'down'}">${money(t.pnl||0)}</div></div><div class="card"><div class="label">סטטוס המנוע</div><div class="v ${r.status==='collecting'?'up':'amber'}">${r.status}</div></div></div>
<h2>עסקאות מדומות אחרונות</h2><div class="scroll"><table><tr><th>זמן</th><th>שוק</th><th>צד</th><th>מחיר מילוי</th><th>הסתברות מודל</th><th>Edge</th><th>מצב</th><th>P&L</th></tr>${d.recent.map(x=>`<tr><td>${new Date(x.ts*1000).toLocaleTimeString('he-IL')}</td><td class="market">${x.market_slug}</td><td class="${x.side==='Up'?'up':'down'}">${x.side}</td><td>${cent(x.fill_price)}</td><td>${pct(x.model_p)}</td><td>${pct(x.edge)}</td><td>${x.status}</td><td>${x.pnl==null?'—':money(x.pnl)}</td></tr>`).join('')}</table></div>${r.last_error?'<p class="err">'+r.last_error+'</p>':''}`;chart(document.getElementById('btcChart'),rows,['btc','open_btc'],['#4ea1ff','#ffd166']);chart(document.getElementById('polyChart'),rows,['up_ask','down_ask','p_up'],['#45e69a','#ff6575','#b584ff'],true)}
